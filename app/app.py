...
def image_creator_page():
    ...
    with concept_tab:
        ...
        with col2:
            ...
            # Add a collapsible section for Campaign Options
            with st.expander("Campaign Options ▾", expanded=False):
                campaign_name = st.text_input(
                    "Campaign Name", 
                    placeholder="Enter the name of this campaign",
                    help="This will help organize your content by campaign"
                )
                ...
                if campaign_goal == "Other":
                    campaign_goal = st.text_input(
                        "Specify Campaign Goal",
                        placeholder="Describe the campaign goal"
                    )
                
                campaign_timeframe = st.date_input(
                    "Campaign Timeframe",
                    help="When will this campaign run?"
                )
                
                # Store campaign info in session state
                if campaign_name and campaign_goal != "Select a goal":
    ...
    with image_tab:
        ...
        with col1:
            st.markdown("""
            <p style="margin-bottom: 5px; font-size: 0.9rem; color: #666;">Choose the image generation model:</p>
            """, unsafe_allow_html=True)
            
            # Add model selection
            model_options = {
                "V_2_TURBO": "V_2_TURBO (High-quality advertising images)",
                "V_1_TURBO": "V_1_TURBO (Fast generation)"
            }
        ...
        with col2:
            ...
            st.markdown("""
            <p style="margin-bottom: 5px; font-size: 0.9rem; color: #666;">Select how many variations to generate:</p>
            """, unsafe_allow_html=True)
        ...
        with st.expander("Advanced Image Options ▾", expanded=False):
            aspect_ratio_col, negative_prompt_col = st.columns(2)
            
            with aspect_ratio_col:
                st.markdown("""
            ...
            with negative_prompt_col:
                st.markdown("""
                <p style="margin-bottom: 5px; font-size: 0.9rem; color: #666;">Elements to avoid in the image:</p>
                """, unsafe_allow_html=True)
                ...
                if 'negative_prompt' in st.session_state and st.session_state.negative_prompt:
                    negative_prompt = st.text_area(
                        ...
                        value=st.session_state.negative_prompt,
                        height=80,
                        help="Elements you want to avoid in the image."
                    )
                else:
                    negative_prompt = st.text_area(
                        "Negative Prompt",
                        placeholder="Example: blurry, distorted text, watermarks, unrealistic proportions",
                        height=80,
                        help="Elements you want to avoid in the image."
                    )
    ...
    with storyboard_tab:
        ...
        if 'ideogram_prompt' in st.session_state:
            ...
            # Main storyboard settings
            col1, col2 = st.columns(2)
            
            with col1:
                st.markdown("""
                <p style="margin-bottom: 5px; font-size: 0.9rem; color: #666;">Choose model for generation:</p>
                """, unsafe_allow_html=True)
            ...
            with col2:
                st.markdown("""
                <p style="margin-bottom: 5px; font-size: 0.9rem; color: #666;">Basic storyboard configuration:</p>
                """, unsafe_allow_html=True)
                
                # Number of frames (always 4 for storyboard)
                st.markdown("""
                <div style="border: 1px solid #ddd; border-radius: 5px; padding: 10px; background-color: #f5f5f5;">
                <p style="margin: 0; font-size: 0.9rem;"><strong>Number of frames:</strong> 4</p>
                <p style="margin: 5px 0 0 0; font-size: 0.8rem; color: #666;">Your storyboard will have 4 sequential images</p>
                </div>
                """, unsafe_allow_html=True)
            
            # Advanced options in a collapsible section
            with st.expander("Advanced Storyboard Options ▾", expanded=False):
                aspect_ratio_col, negative_prompt_col = st.columns(2)
                
                with aspect_ratio_col:
                    st.markdown("""
                    <p style="margin-bottom: 5px; font-size: 0.9rem; color: #666;">Choose dimensions for all frames:</p>
                    """, unsafe_allow_html=True)
                    ...
                    aspect_ratio_selected = st.selectbox(
                        ...
                        help="Select the shape of your storyboard images. Square (1:1) is good for social media posts."
                    )
                    aspect_ratio = aspect_ratio_options[aspect_ratio_selected]
                
                with negative_prompt_col:
                    st.markdown("""
                    <p style="margin-bottom: 5px; font-size: 0.9rem; color: #666;">Elements to avoid in all frames:</p>
                    """, unsafe_allow_html=True)
